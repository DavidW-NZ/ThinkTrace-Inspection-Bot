...
class TelegramBridgeTests(unittest.TestCase):
    ...
    def test_fetch_inspection_setups_raises_unmapped_user_error(self):
        config = telegram_bridge.TelegramBridgeConfig(
            base_url="https://example.test/",
            token="bridge-secret",
        )
            ...
            return_value=_FakeResponse(
                {
                    ...
                    "error": {
                        ...
                        "message": "Telegram user is not mapped.",
                    },
                }
            ),
        ):
            with self.assertRaises(telegram_bridge.TelegramUserNotMappedError):
                telegram_bridge.fetch_inspection_setups(
                    telegram_user_id=123,
                    config=config,
                )
